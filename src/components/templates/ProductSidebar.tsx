    ...
    <aside className="w-full md:w-1/6 p-4 bg-white max-w-max">
      ...
      {categories.map(({ type, brands }) => (
        <div key={type} className="mb-4 pl-2">
          <h3 className="font-normal text-sm text-textGrey">{type}</h3>
            ...
            <div key={name} className="flex items-center mb-1">
              <input
                ...
                checked={selectedBrands.some(
                  (entry) =>
                    entry.category === type && entry.brands.includes(name)
                )}
                onChange={() => handleToggle(setSelectedBrands, name, type)}
                className="mr-2"
              />
              <label className="font-normal text-sm text-textGrey">
                {name}
              </label>
      ...
      <div className="flex items-center">
        ...
        />
        <FontAwesomeIcon icon={faChevronUp} className="text-primaryGreen" />
      </div>
      ...
      {suppliers.map((name) => (
        <div key={name} className="flex items-center mb-1">
          <input
            ...
            onChange={() => handleToggle(setSelectedSuppliers, name)}
            className="mr-2"
          />
          <label className="font-normal text-sm text-textGrey">{name}</label>
        </div>
      ))}
      <div className="flex items-center">
        <CustomButton
      ...
      <div className="flex flex-wrap gap-2">
        {keywords.map((kw) => (
          <button
            key={kw}
            className={`px-3 py-1 rounded-full text-sm ${selectedKeywords.includes(kw) ? 'bg-blue-500 text-white' : 'bg-backgroundGrey'}`}

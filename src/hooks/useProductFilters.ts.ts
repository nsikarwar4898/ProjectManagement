...
const useProductFilters = (products: any[]) => {
  ...
  const handleToggle = (setState: any, value: string, category = '') => {
    if (category.length) {
      setState((prevSelected: { category: string; brands: string[] }[]) => {
        const categoryEntry = prevSelected.find(
          (entry) => entry.category === category
        );

        if (categoryEntry) {
          const updatedBrands = categoryEntry.brands.includes(value)
          ...
          return updatedBrands.length
            ...
            : prevSelected.filter((entry) => entry.category !== category);
        } else {
  ...
  const filteredProducts = products.filter((product) => {
    return (
      ...
      (selectedBrands.length === 0 ||
        selectedBrands.some(
          ({ category, brands }) =>
            product.category === category && brands.includes(product.brand)
        )) &&
